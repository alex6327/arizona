...
//            die();
    ...
    function listing($grpId, $catId, $ssn) {
        if ($catId == 12 or $ssn == 3916 or $ssn == 3919) {
            $data = array();
            $this->load->model('products_model');
            $data['products'] = $this->products_model->get_pcr($catId, $ssn);
            $grpName = $this->products_model->get_grpName($grpId);
            $data['catName'] = $this->products_model->get_catName($grpName, $catId);
            $data['subCatName'] = $this->products_model->get_subCatName($grpName . "_" . $catId, $ssn);
            $data['pagetitle'] = '产品列表';
            ...
            $data['ssn'] = $ssn;
            $data['pagetitle'] = '产品列表';
            $data['pagebody'] = 'product/pcr_listing';
            $data['sitenavi'] = 'product/list_sitenavi';
            $data['data'] = &$data;
            $this->load->view('template', $data);
        } else {
    ...
    function category($grpId, $catId) {
        if ($catId == 12) {
            $data = array();
            $data['pagetitle'] = 'PCR,RT-PCR & 荧光定量PCR';
            $data['pagebody'] = 'product/pcr';
            $data['sitenavi'] = 'product/pcr_sitenavi';
            $data['data'] = &$data;
            $this->load->view('template', $data);
        } else {
